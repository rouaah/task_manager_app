...
class LoggerInterceptor implements Interceptor {
  ...
  void _logHeaders({required Map headers, Styles? style}) {
    ...
    headers.forEach(
      (key, value) {
        _log(
          key: '\t$key: ',
          value: (value is List && value.length == 1) ? value.first : value.toString(),
          style: style,
        );
      },
    );
  ...
  void _logRequest(RequestOptions options, {Styles? style}) async {
    ...
    _log(
        key: 'Extra: ',
        value:
            ' ${options.extra.entries.isNotEmpty && options.extra.entries.elementAt(0).value == false ? 'Token is not Required' : '${await SecureStorage().getAccessToken()}'}',
        style: style);
  ...
  void _logError(DioException err, {Styles? style}) {
    _log(key: 'Error -> ', value: err.toString(), style: style);
    _log(
      key: 'DioErrorType: ',
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    //we must convert it  to Map<String, dynamic> cause here options.data type is internallinkedhashmap<string,int>
    // will throw errors say string isn't a subtype of int value on all app requests when inject data below
    //except if we pass lang/token/customerID/Auth from repo layer will work fine
